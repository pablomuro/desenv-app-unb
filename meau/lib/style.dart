import 'package:flutter/material.dart';

//Text Sizes Constants
const LargeTextSize = 72.0;
const LargeMediumTextSize = 53.0;
const MediumTextSize = 16.0;
const TextSize14 = 14.0;
const SmallTextSize = 12.0;

const AppTitleTextSize = 20.0;

const String CourgettFontFamily = 'Courgette';
const String RobotoFontFamily = 'Roboto';

//Color constant
const Color DefaultStrongreGrennColor = Color(0xff589b9b);
const Color DefaultGrennColor = Color(0xff88c9bf);
const Color DefaultYellowColor = Color(0xffffd358);
const Color DefaultLightYellowColor = Color(0xfffee29b);
const Color DefaultTextColor = Color(0xff757575);
const Color DefaultButtonColor = Color(0xff434343);

const Color MenuInfoBgColor = Color(0xffcfe9e5);
const Color MenuConfigBgColor = Color(0xffe6e7e8);
const Color MenuTileBgColor = Color(0xfff7f7f7);

const Color ImagePickerGray = Color(0xffe6e7e7);

const Color FacebookBtnColor = Color(0xff194f7c);
const Color GoogleBtnColor = Color(0xfff15f5c);


const Text14DefaultStyle = TextStyle(
  fontFamily: RobotoFontFamily,
  fontSize: TextSize14,
  color: DefaultTextColor
);


const TextWelcomeStyle = TextStyle(
  fontFamily: CourgettFontFamily,
  fontSize: LargeTextSize,
  color: DefaultYellowColor
);

const TextOpsStyle = TextStyle(
  fontFamily: CourgettFontFamily,
  fontSize: LargeMediumTextSize,
  color: DefaultGrennColor
);


const TextIntroStyle = TextStyle(
  fontSize: MediumTextSize,
  color: DefaultTextColor
);

const TextYellownFlatButtonStyle = TextStyle(
  fontSize: SmallTextSize,
  color: DefaultButtonColor
);
